...
//define('ALTAPAY_PLUGIN_VERSION', '3.2');
...
class ControllerExtensionModuleAltapay extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate() && isset($_POST['terminalrefresh'])) {
			if(!$this->config->get('module_altapay_gateway_url') || !$this->config->get('module_altapay_gateway_username') || !$this->config->get('module_altapay_gateway_password')){
				...
				require_once($upOne.'/altapay/altapay/altapay-php-sdk/lib/AltapayMerchantAPI.class.php');
				$api = new AltapayMerchantAPI($this->config->get('module_altapay_gateway_url'), $this->config->get('module_altapay_gateway_username'), $this->config->get('module_altapay_gateway_password'), null);
				$response = $api->login();
				
				if($response->wasSuccessful()) {
					$data['refresh_terminals_success'] = $this->language->get('text_refresh_success');
					$terminals = $api->getTerminals();
					
					foreach ($terminals->getTerminals() as $key => $term) {
						...
						$term_name = str_replace($this->forbiddenChars,"", $term->getTitle());
					...
					//echo '<pre>'.print_r($_terminals, true).'</pre>';
			...
			//$this->response->redirect($this->url->link('module/altapay', 'user_token=' . $this->session->data['user_token'], true));
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/altapay', 'user_token=' . $this->session->data['user_token'], true),
			'separator' => ' :: '
		);
	...
	public function install() {
		...
		// $this->model_extension_event->addEvent('altapay', 'catalog/controller/account/logout/after', 'extension/module/altapay/logout');
	...
	public function uninstall() {
		// $this->load->model('extension/event');
		// $this->model_extension_event->deleteEvent('altapay');
