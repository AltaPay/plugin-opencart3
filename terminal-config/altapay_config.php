...
{
    $query = $db->query("SELECT * FROM " . DB_PREFIX . "setting WHERE `key` = '" . $this->db->escape((string)$key) . "'");

    if ($query->num_rows) {
        $db->query("UPDATE " . DB_PREFIX . "setting SET `code` = '" . $this->db->escape((string)$code) . "', `value` = '" . $this->db->escape((string)$value) . "' WHERE `key` = '" . $this->db->escape((string)$key) . "'");
    } else {
        $db->query("INSERT INTO  " . DB_PREFIX . "setting  (`store_id`,`code`,`key`, `value`,`serialized`) VALUES(0, '" . $this->db->escape((string)$code) . "', '" . $this->db->escape((string)$key) . "', '" . $this->db->escape((string)$value) . "', 0)");
    }
...
{
    $query = $db->query("SELECT * FROM " . DB_PREFIX . "extension WHERE `code` = '" . $this->db->escape((string)$code) . "'");
    if ($query->num_rows == 0) {
        $db->query("INSERT INTO  " . DB_PREFIX . "extension  (`type`,`code`) VALUES('" . $this->db->escape((string)$type) . "', '" . $this->db->escape((string)$code) . "')");
    }
...
{
    $query = $db->query("SELECT `currency_id` FROM " . DB_PREFIX . "currency WHERE `code` = '" . $this->db->escape((string)$code) . "'");
    if ($query->num_rows) {
        $db->query("UPDATE " . DB_PREFIX . "currency  SET `title` = '" . $this->db->escape((string)$title) . "', `symbol_left` = '" . $this->db->escape((string)$symbol) . "', `date_modified` = NOW() WHERE `code` = '" . $this->db->escape((string)$code) . "'");
        return $query->row['currency_id'];
    } else {
        $db->query("INSERT INTO " . DB_PREFIX . "currency (`title`, `code`, `symbol_left`, `symbol_right`, `decimal_place`, `value`, `status`, `date_modified`) VALUES ('" . $this->db->escape((string)$title) . "', '" . $this->db->escape((string)$code) . "', '" . $this->db->escape((string)$symbol) . "', '', '2', '1.00000000', '1', NOW())");
        return $db->getLastId();
    }
}
